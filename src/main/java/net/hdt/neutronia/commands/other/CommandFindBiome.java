package net.hdt.neutronia.commands.other;
...
public class CommandFindBiome extends CommandBase {
	...
	public String getName() {
		return "findbiome_test";
	}

	@Override
	public String getUsage(ICommandSender sender) {
		return "/findbiome_test <biome>";
	}
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) {
		...
		new Thread(() -> {
			BlockPos pos = spiralOutwardsLookingForBiome(sender, sender.getEntityWorld(), finalBiome, sender.getPosition().getX(), sender.getPosition().getZ(), 10_000);
			if (pos == null) {
...
//					playerMP.connection.setPlayerLocation(pos.getX(), 150, pos.getZ(), 0, 0);
					sender.sendMessage(new TextComponentString(TextFormatting.WHITE + "Found '" + finalBiome.getRegistryName().toString() + String.format("' Biome! Position: X: %s, Z: %s", pos.getX(), pos.getZ())));
				});
				return;
			}
