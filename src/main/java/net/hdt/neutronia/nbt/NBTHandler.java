package net.hdt.neutronia.nbt;

import net.hdt.neutronia.Main;
import net.hdt.neutronia.nbt.parser.INBTParser;
import net.hdt.neutronia.nbt.parser.NBTParsers;
...
public enum NBTHandler {
    ...
    private <V, T extends NBTBase> T writeFieldToNBT(V object, Field field) {
        ...
        try {
            ...
            Main.LOGGER.fatal(CrashReport.makeCrashReport(e.getCause(), e.getCause().getLocalizedMessage()).getCompleteReport());
            return null;
        }
    ...
    private Method resolveMutator(Class<?> clazz, String name, Class<?>... param) {
        ...
        if (method == null) {
            ...
            Main.LOGGER.fatal(CrashReport.makeCrashReport(new RuntimeException(message), message));
            return null;
        }
