...
public class VariedDungeons {
	...
	private String lootTableStr = "";
	private ResourceLocation lootTable = lootTableStr.isEmpty() ? null : new ResourceLocation(lootTableStr);
			...
			for(int y = 0; y < size.getY(); y++)
				for(int z = 0; z < size.getZ(); z++) {
					...
					if(checkState.getBlock().getBlockHardness(checkState, world, checkPos) == -1 || world.canBlockSeeSky(checkPos)) {
                        System.out.println("Can't generate dungeon");
                        return;
                    }
				}
