...
public class ImprovedSleeping extends Component {
	...
	private static String TAG_AFK = "neutronia:afk";
	...
	public static void updateAfk(EntityPlayer player, boolean afk) {
		...
		if(player.world.playerEntities.size() != 1) {
			if(afk) {
				...
				TextComponentTranslation text = new TextComponentTranslation("neutroniaMisc.nowAfk");
				text.getStyle().setColor(TextFormatting.AQUA);
				player.sendMessage(text);
			} else {
				player.getEntityData().setBoolean(TAG_AFK, false);
				TextComponentTranslation text = new TextComponentTranslation("neutroniaMisc.leftAfk");
				text.getStyle().setColor(TextFormatting.AQUA);
				player.sendMessage(text);
			}
		}
	...
	public static int isEveryoneAsleep(World world) {
		if(!GroupLoader.isFeatureEnabled(ImprovedSleeping.class))
			return 0;
	...
	public void onWorldTick(PlayerTickEvent event) {
		...
		if(!sleeper.isEmpty() && world.playerEntities.size() != 1) {
			...
			TextComponentBase message = new TextComponentTranslation("neutroniaMisc.personSleeping", sleeper);
			...
			TextComponentBase hoverText = new TextComponentTranslation("neutroniaMisc.sleepingListHeader", "\n");
