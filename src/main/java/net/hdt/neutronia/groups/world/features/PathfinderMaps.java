...
public class PathfinderMaps extends Feature {
	...
	private void loadCustomMaps(String[] lines) {
		for(String s : lines)
			try {
				...
				FMLLog.warning("[Neutronia][Custom Pathfinder Maps] Error while reading custom map string \"%s\"", s);
				FMLLog.warning("[Neutronia][Custom Pathfinder Maps] - %s", e.getMessage());
			}
	}
	...
	public static class TradeInfo {
		...
		TradeInfo(String category, Biome biome, boolean enabled, int level, int minPrice, int maxPrice, int color) {
			this(category, biome, enabled, level, minPrice, maxPrice, color, "neutronia.biomeMap." + biome.getRegistryName().getPath());
		}
		
		TradeInfo(String category, Biome biome, boolean enabled, int level, int minPrice, int maxPrice, int color, String name) {
			this.enabled = GroupLoader.config.getBoolean("Enabled", category, enabled, "");
			this.biome = biome;
			this.level = GroupLoader.config.getInt("Required Villager Level", category, level, 0, 10, "");
			this.minPrice = GroupLoader.config.getInt("Minimum Emerald Price", category, minPrice, 1, 64, "");
			this.maxPrice = Math.max(minPrice, GroupLoader.config.getInt("Maximum Emerald Price", category, maxPrice, 1, 64, ""));
			this.color = color;
			this.name = name;
		}
	}
