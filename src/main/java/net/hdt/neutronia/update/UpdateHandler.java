package net.hdt.neutronia.update;
...
public enum UpdateHandler {
    ...
    public void registerUpdateChecker(Object mod, String url) {
        Thread thread = new Thread(() -> {
            if (!mod.getClass().isAnnotationPresent(Mod.class)) {
                Main.LOGGER.warn("Please register the update checker using the main mod class. Skipping registration of object {}.", mod);
                return;
            }
            ...
            try {
                ...
                if (updateContainer != null) {
                    ...
                    if (modContainer[0] == null) {
                        Main.LOGGER.warn("Couldn't find mod container with id {}. Skipping registration of object {}.", annotation.modid(), mod);
                        return;
                    }
                    ...
                    Main.LOGGER.warn("Failed to load update container for mod {} ({})!", annotation.name(), annotation.modid());
                }
    ...
    /**
     * Search for mod updates. This hook is getting called by {@link net.hdt.neutronia.proxy.CommonProxy#postInit(FMLPostInitializationEvent)}
     */
    @Deprecated
